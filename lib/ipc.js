...
module.exports = function(synapps) {
    ...
    if (synapps.isMaster) {
      ...
      if (synapps._config.tls && synapps._config.tls.key && synapps._config.tls.cert) {
        ...
        .parallel().asCallback(function(err, files) {
          ...
          var clusterSock = null;
          try {
            clusterSock = self.socket.listen('tls://' + host + ':' + port, tlsOptions);
          } catch(e) {
            var err = new Error('Could not initialize cluster tls connections');
            err.original = e;
          ...
          self.socket.on('cluster.addCa', function(ca) {
            clusterSock.addCa(ca);
          });
        });
      }
      ...
      this.socket.on('register', function(identity) {
        if (identity.startsWith('worker')) {
          ...
          synapps.emit('worker.register', identity);
      ...
      this.socket.on('debug.state', function(defer) {
        var os = require('os');
      ...
      this.socket.on('debug.stop', function(defer) {
        inspector.close();
        synapps.debug('info', 'worker leave debug mode');
        defer.resolve({});
      });

      // master ask worker to die
      this.socket.on('synapps.kill', function() {
        synapps.debug('debug', synapps._config.name + ' received kill signal');
  ...
  ipc.prototype.send = function(node, req, requestErrorHandler) {
    ...
    if (requestErrorHandler) {
      ...
      defer.resolve = requestErrorHandler.bind(defer.resolve);
      defer.reject = requestErrorHandler.bind(defer.reject);
      defer.notify = requestErrorHandler.bind(defer.notify);

      this.socket.defer('master', 'request', { host: node, req: req }).done(function(res) {
        defer.resolve(res);
      }).fail(function(err) {
        defer.reject(err);
    ...
    if (synapps.isMaster) {
      defer = this.socket.defer(node, 'request', req);
    } else {
      defer = this.socket.defer('master', 'request', { host: node, req: req });
    }
    return defer;
  };
