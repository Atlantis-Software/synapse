...
module.exports = function(synapps) {
  ...
  if (!synapps) {
    var synapps = {debug: function(alertLevel, message) { console.log(message) }};
  }
  ...
  function Socket() {
    ...
    this.on('connect', function(){
      ...
      var len = prev.length;
      self.queue = [];
      synapps.debug('trace', 'flush ' + len + ' messages');
  ...
  */
  ...
  Socket.prototype.closeSockets = function(fn){
    synapps.debug('trace', this.type + ' closing ' + this.socks.length + ' connections');
  ...
  Socket.prototype.close = function(fn){
    synapps.debug('trace', this.type + ' closing');
    this.closing = true;
  ...
  Socket.prototype.closeServer = function(fn){
    synapps.debug('trace', this.type + ' closing server');
  ...
  Socket.prototype.removeSocket = function(sock){
    ...
    if (!~i) return;
    synapps.debug('trace', this.type + ' remove socket ' + i);
  ...
  Socket.prototype.addSocket = function(sock){
    ...
    synapps.debug('trace', this.type + ' add socket ' + i);
    sock.pipe(parser);
  ...
  Socket.prototype.handleErrors = function(sock){
    ...
    sock.on('error', function(err){
      ...
      synapps.debug('error', self.type + ' error ' + err.code || err.message);
      self.emit('socket error', err);
      self.removeSocket(sock);
      if (!~ignore.indexOf(err.code)) return self.emit('error', err);
      synapps.debug('trace', self.type + ' ignored ' + err.code);
  ...
  Socket.prototype.onmessage = function(sock){
    ...
    return function (buf){
      ...
      if (task === 'registered') {
        ...
        if (!fn) return synapps.debug('error', 'missing callback ' + id);
      ...
      function emit() {
        ...
        if (sock.writable) {
          ...
          synapps.debug('trace', 'peer went away');
  ...
  Socket.prototype.connect = function(identity, port, host, fn){
    ...
    var onConnect = function() {
      synapps.debug('trace', self.type + ' connect');
      self.connected = true;
      self.addSocket(sock);
    ...
    if (tlsOpts) {
      tlsOpts.host = host;
      tlsOpts.port = port;
      synapps.debug('trace', self.type + ' connect attempt ' + host + ':' + port);
    ...
    var onError = function(err) {
      if (err.code === 'ECONNREFUSED') {
        ...
        setTimeout(function(){
          synapps.debug('trace', self.type + ' attempting reconnect');
  ...
  Socket.prototype.register = function(identity, host, fn) {
    ...
    this.connect(identity, port, host, function(err, sock) {
      ...
      if (sock) {
        ...
        synapps.debug('trace', 'no connected peers');
        self.enqueue(args);
      }
    });
  };
  ...
  Socket.prototype.onconnect = function(sock){
    ...
    if (tlsOptions && !sock.authorized) {
      synapps.debug('error', self.type + ' denied ' + addr + ' for authorizationError ' + sock.authorizationError);
    }
    synapps.debug('debug', self.type + ' accept ' + addr);
    this.addSocket(sock);
    this.handleErrors(sock);
    this.emit('connect', sock);
    sock.on('close', function() {
      synapps.debug('debug', self.type + ' disconnect ' + addr);
  ...
  Socket.prototype.bind = function(port, host, fn){
    ...
    synapps.debug('trace', this.type + ' bind ' + host + ':' + port);
  ...
  Socket.prototype.netSend = function(sock, task, data, cb) {
    var id = this.id();
    var args = Array.prototype.slice.call(arguments);
    args.shift();
    args[0] = args[0] || null;
    ...
    var hasCallback = 'function' == typeof args[args.length - 1];
    if (hasCallback) cb = args.pop();
    args.push(id);

    if (sock.writable) {
      sock.on('data', function(resp) {
        var response = JSON.parse(resp.toString());
        if (response.error) {
      ...
      synapps.debug(5, 'peer went away');
      return cb('peer went away');
    }
  };
