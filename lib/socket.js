...
module.exports = function(synapps) {
  ...
  function Socket() {
    ...
    this.on('connect', function(){
      ...
      var len = prev.length;
      self.queue = [];
      synapps.debug(5, 'flush ' + len + ' messages');
  ...
  */
  ...
  Socket.prototype.closeSockets = function(fn){
    synapps.debug(5, this.type + ' closing ' + this.socks.length + ' connections');
  ...
  Socket.prototype.close = function(fn){
    synapps.debug(5, this.type + ' closing');
    this.closing = true;
  ...
  Socket.prototype.closeServer = function(fn){
    synapps.debug(5, this.type + ' closing server');
  ...
  Socket.prototype.removeSocket = function(sock){
    ...
    if (!~i) return;
    synapps.debug(5, this.type + ' remove socket ' + i);
  ...
  Socket.prototype.addSocket = function(sock){
    ...
    synapps.debug(5, this.type + ' add socket ' + i);
    sock.pipe(parser);
  ...
  Socket.prototype.handleErrors = function(sock){
    ...
    sock.on('error', function(err){
      ...
      synapps.debug(5, self.type + ' error ' + err.code || err.message);
      self.emit('socket error', err);
      self.removeSocket(sock);
      if (!~ignore.indexOf(err.code)) return self.emit('error', err);
      synapps.debug(5, self.type + ' ignored ' + err.code);
  ...
  Socket.prototype.onmessage = function(sock){
    ...
    return function (buf){
      ...
      if (task === 'registered') {
        ...
        if (!fn) return synapps.debug(5, 'missing callback ' + id);
  ...
  Socket.prototype.connect = function(identity, port, host, fn){
    ...
    var onConnect = function() {
      synapps.debug(5, self.type + ' connect');
      self.connected = true;
      self.addSocket(sock);
    ...
    if (tlsOpts) {
      tlsOpts.host = host;
      tlsOpts.port = port;
      synapps.debug(5, self.type + ' connect attempt ' + host + ':' + port);
    ...
    var onError = function(err) {
      if (err.code === 'ECONNREFUSED') {
        ...
        setTimeout(function(){
          synapps.debug(5, self.type + ' attempting reconnect');
  ...
  Socket.prototype.register = function(identity, host, fn) {
    ...
    this.connect(identity, port, host, function(err, sock) {
      ...
      if (sock) {
        ...
        synapps.debug(5, 'no connected peers');
        self.enqueue(args);
      }
    });
  };
  ...
  Socket.prototype.onconnect = function(sock){
    ...
    if (tlsOptions && !sock.authorized) {
      synapps.debug(5, self.type + ' denied ' + addr + ' for authorizationError ' + sock.authorizationError);
    }
    synapps.debug(5, self.type + ' accept ' + addr);
    this.addSocket(sock);
    this.handleErrors(sock);
    this.emit('connect', sock);
    sock.on('close', function() {
      synapps.debug(5, self.type + ' disconnect ' + addr);
  ...
  Socket.prototype.bind = function(port, host, fn){
    ...
    synapps.debug(5, this.type + ' bind ' + host + ':' + port);
