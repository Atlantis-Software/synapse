...
module.exports = function(synapps) {
  return function(event) {
    ...
    if (validation) {
      ...
      // request is invalid
      synapps.debug(2, 'Invalid Request for route `' + req.request + '`');
    ...
    req.done(function() {
      synapps.debug(5, util.format('Request OK for route `%s` after %s ms on worker `%s` with pid %s', req.request, (req.end - req.start), process.env.WORKER_NAME, process.pid));
    });

    req.fail(function() {
      synapps.debug(5, util.format('Request ERROR for route `%s` after %s ms on worker `%s` with pid %s', req.request, (req.end - req.start), process.env.WORKER_NAME, process.pid));
      if (req.requestErrorHandler.executionError) {
        return synapps.debug(5, req.requestErrorHandler.executionError);
      }
    });
