...
module.exports = function(synapps) {
  return function(event) {
    ...
    req.socket = {
      join: function(room) {
        synapps._ipc.relations['master'].emit('socket.join', {socketID: event.data.socketID, room: room});
      },
      leave: function(room) {
        synapps._ipc.relations['master'].emit('socket.leave', {socketID: event.data.socketID, room: room});
      },
      to: function(room) {
        return {
          emit: function(eventName, message) {
            synapps._ipc.relations['master'].emit('socket.to.emit', {room: room, event: eventName, msg: message});
          }
        };
      }
    };
